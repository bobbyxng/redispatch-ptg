...
η_E = parse(Float64, ARGS[4])
η_M = parse(Float64, ARGS[5])

timeperiod_str1 = replace(string(timeperiod), ":" => "-")
timeperiod_str = timeperiod_str1*"__"*ARGS[4]*"__"*ARGS[5]
...
CM_PtG_mod, CM_PtG_ΔP_up_opt, CM_PtG_ΔP_dn_opt, CM_PtG_ΔP_R_up_opt, CM_PtG_ΔP_R_dn_opt,
	CM_PtG_Θ_opt, CM_PtG_P_flow_opt, CM_PtG_price, CM_PtG_P_gen_lost_opt, CM_PtG_P_load_lost_opt,
	CM_PtG_P_S_up_opt, CM_PtG_P_syn_opt, CM_PtG_D_PtG_opt, CM_PtG_L_syn_opt = CongestionManagementPHS80PtG(dayslicer(timeperiod)[1],
	nodes, lines, pp, res, storages, ED_price, ED_P_opt, ED_P_R_opt_dist,
	ED_P_S_opt, ED_D_S_opt, ED_L_S_opt, η_E, η_M)
...
next!(prog)
	...
	# Dispatchable power plants
	output_CM_PtG_ΔP_up_opt = DataFrame(time=Int64[],unit=Symbol[],fuel=String[],output=Float64[],node=Int64[])
	try
		for t in dayslicer(timeperiod)[1]
			for g in pp.unit
				val = CM_PtG_ΔP_up_opt[t, g]
				fuel =  pp.fuel[g]
				push!(output_CM_PtG_ΔP_up_opt, [t g fuel val pp.node[g]])
			end
		end
		CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_P_up.csv"), output_CM_PtG_ΔP_up_opt)
	catch e
		println("CM_PtG_ΔP_up_opt does not exist.")
	end

	output_CM_PtG_ΔP_dn_opt = DataFrame(time=Int64[],unit=Symbol[],fuel=String[],output=Float64[],node=Int64[])
	try
		for t in dayslicer(timeperiod)[1]
			for g in pp.unit
				val = -1*CM_PtG_ΔP_dn_opt[t, g]
				fuel =  pp.fuel[g]
				push!(output_CM_PtG_ΔP_dn_opt, [t g fuel val pp.node[g]])
			end
		end
		CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_P_dn.csv"), output_CM_PtG_ΔP_dn_opt)
	catch e
		println("CM_PtG_ΔP_dn_opt does not exist.")
	end

	# RES generation
	output_CM_PtG_ΔP_R_up_opt = DataFrame(time=Int64[],unit=Symbol[],fuel=Symbol[],output=Float64[],node=Int64[])
	try
		for t in dayslicer(timeperiod)[1]
			for r in res.unit
				val = CM_PtG_ΔP_R_up_opt[t, r]
				fuel =  res.fuel[r]
				push!(output_CM_PtG_ΔP_R_up_opt, [t r fuel val res.node[r]])
			end
		end
		CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_P_R_up.csv"), output_CM_PtG_ΔP_R_up_opt)
	catch e
		println("CM_PtG_ΔP_R_up_opt does not exist.")
	end

	output_CM_PtG_ΔP_R_dn_opt = DataFrame(time=Int64[],unit=Symbol[],fuel=Symbol[],output=Float64[],node=Int64[])
	try
		for t in dayslicer(timeperiod)[1]
			for r in res.unit
				val = -1*CM_PtG_ΔP_R_dn_opt[t, r]
				fuel =  res.fuel[r]
				push!(output_CM_PtG_ΔP_R_dn_opt, [t r fuel val res.node[r]])
			end
		end
		CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_P_R_dn.csv"), output_CM_PtG_ΔP_R_dn_opt)
	catch e
		println("CM_PtG_ΔP_R_dn_opt does not exist.")
	end

	### Model parameters
	output_CM_PtG_info = DataFrame(timeperiod=String[],
									   termination_status=String[],
									   primal_status=String[],
									   dual_status=String[],
									   objective_value=Float64[],
	...
	push!(output_CM_PtG_info, [string(dayslicer(timeperiod)[1]) string(termination_status(CM_PtG_mod)) string(primal_status(CM_PtG_mod)) string(dual_status(CM_PtG_mod)) objective_value(CM_PtG_mod) solve_time(CM_PtG_mod)])
	CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_info.csv"), output_CM_PtG_info)

	### Power flow results
	output_CM_PtG_P_flow_opt = DataFrame(time=Int64[],
									 line=Int64[],
									 from=Int64[],
									 to=Int64[],
									 pmax=Float64[],
	 ...
	 try
 		for t in dayslicer(timeperiod)[1]
 			for l in lines.id
				...
				pmax = lines.pmax[l]
				pflow = CM_PtG_P_flow_opt[t, l]
 				push!(output_CM_PtG_P_flow_opt, [t l from to pmax pflow])
 			end
 		end
 		CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_P_flow.csv"), output_CM_PtG_P_flow_opt)
 	catch e
 		println("CM_PtG_P_flow_opt does not exist.")
 	end

	# Lost generation
	output_CM_PtG_P_gen_lost = DataFrame(time=Int64[],node=Int64[],output=Float64[])
	try
		for t in dayslicer(timeperiod)[1]
			for j in nodes.id
				output = CM_PtG_P_gen_lost_opt[t, j]
				push!(output_CM_PtG_P_gen_lost, [t, j, output])
			end
		end
		CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_P_gen_lost.csv"), output_CM_PtG_P_gen_lost)
	catch e
		println("CM_PtG_P_gen_lost does not exist.")
	end

	# Lost load
	output_CM_PtG_P_load_lost = DataFrame(time=Int64[],node=Int64[],output=Float64[])
	try
		for t in dayslicer(timeperiod)[1]
			for j in nodes.id
				output = CM_PtG_P_load_lost_opt[t, j]
				push!(output_CM_PtG_P_load_lost, [t, j, output])
			end
		end
		CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_P_load_lost.csv"), output_CM_PtG_P_load_lost)
	catch e
		println("CM_PtG_P_load_lost does not exist.")
	end

	output_CM_PtG_price = DataFrame(time=Int64[],node=Int64[],price=Float64[])
	 try
		 for t in dayslicer(timeperiod)[1]
			 for j in nodes.id
				 price =  CM_PtG_price[j, t]
				 push!(output_CM_PtG_price, [t j price])
			 end
		 end
		 CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_price.csv"), output_CM_PtG_price)
	 catch e
		 println("CM_PtG_price does not exist.")
	 end

	# PHS output increase
	output_CM_PtG_P_S_up_opt = DataFrame(time=Int64[],unit=Symbol[],fuel=String[],output=Float64[],node=Int64[])
	try
		for t in dayslicer(timeperiod)[1]
			for s in storages.unit
				val = CM_PtG_P_S_up_opt[t, s]
				fuel =  storages.fuel[s]
				push!(output_CM_PtG_P_S_up_opt, [t s fuel val storages.node[s]])
			end
		end
		CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_P_S_up.csv"), output_CM_PtG_P_S_up_opt)
	catch e
		println("CM_PtG_P_S_up_opt does not exist.")
	end

	# Electricity generation from synthetic methane
	output_CM_PtG_P_syn_opt = DataFrame(time=Int64[],unit=Symbol[],fuel=String[],output=Float64[],node=Int64[])
	try
		for t in dayslicer(timeperiod)[1]
			for e in getKeyVector(pp.fuel, "NaturalGas")
				val = CM_PtG_P_syn_opt[t, e]
				fuel = "SNG"
				push!(output_CM_PtG_P_syn_opt, [t e fuel val pp.node[e]])
			end
		end
		CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_P_syn.csv"), output_CM_PtG_P_syn_opt)
	catch e
		println("CM_PtG_P_syn_opt does not exist.")
	end

	# Electricity demand from PtG units
	output_CM_PtG_D_PtG_opt = DataFrame(time=Int64[],unit=Symbol[],fuel=String[],output=Float64[],node=Int64[])
	try
		for t in dayslicer(timeperiod)[1]
			for r in CM_PtG_D_PtG_opt.axes[2]
				val = CM_PtG_D_PtG_opt[t, r]
				fuel = "PtG"
				push!(output_CM_PtG_D_PtG_opt, [t r fuel val res.node[r]])
			end
		end
		CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_D_PtG.csv"), output_CM_PtG_D_PtG_opt)
	catch e
		println("CM_PtG_D_PtG_opt does not exist.")
	end
	...
	try
		...
		CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_L_syn.csv"), output_CM_PtG_L_syn_opt)
	catch e
		println("CM_PtG_L_syn_opt does not exist.")
	end
...
for hours in dayslicer(timeperiod)[2:end]
	...
	global ED_mod, ED_P_opt, ED_P_R_opt, ED_P_R_opt_dist, ED_P_S_opt, ED_D_S_opt,
		...
		println("Calculating congestion management: " * string(hours))
			...
			global CM_PtG_mod, CM_PtG_ΔP_up_opt, CM_PtG_ΔP_dn_opt, CM_PtG_ΔP_R_up_opt, CM_PtG_ΔP_R_dn_opt,
				CM_PtG_Θ_opt, CM_PtG_P_flow_opt, CM_PtG_price, CM_PtG_P_gen_lost_opt, CM_PtG_P_load_lost_opt,
				CM_PtG_P_S_up_opt, CM_PtG_P_syn_opt, CM_PtG_D_PtG_opt, CM_PtG_L_syn_opt = CongestionManagementPHS80PtG(hours, nodes, lines, pp, res, storages, ED_price,
				ED_P_opt,  ED_P_R_opt_dist, ED_P_S_opt, ED_D_S_opt, ED_L_S_opt, η_E, η_M)
			...
			try
				...
				CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_P_up.csv"), output_CM_PtG_ΔP_up_opt, append = true)
			catch e
				println("CM_PtG_ΔP_up_opt does not exist.")
			end
			...
			try
				...
				CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_P_dn.csv"), output_CM_PtG_ΔP_dn_opt, append = true)
			catch e
				println("CM_PtG_ΔP_dn_opt does not exist.")
			end
			...
			try
				...
				CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_P_R_up.csv"), output_CM_PtG_ΔP_R_up_opt, append = true)
			catch e
				println("CM_PtG_ΔP_R_up_opt does not exist.")
			end
			...
			try
				...
				CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_P_R_dn.csv"), output_CM_PtG_ΔP_R_dn_opt, append = true)
			catch e
				println("CM_PtG_ΔP_R_dn_opt does not exist.")
			end
			...
			push!(output_CM_PtG_info, [string(hours) string(termination_status(CM_PtG_mod)) string(primal_status(CM_PtG_mod)) string(dual_status(CM_PtG_mod)) objective_value(CM_PtG_mod) solve_time(CM_PtG_mod)])
			CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_info.csv"), output_CM_PtG_info, append = true)
			...
			try
			   ...
			   CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_P_flow.csv"), output_CM_PtG_P_flow_opt, append = true)
		   catch e
			   println("CM_PtG_P_flow_opt does not exist.")
		   end
		   ...
		   try
			   ...
			   CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_P_gen_lost.csv"), output_CM_PtG_P_gen_lost, append = true)
		   catch e
			   println("CM_PtG_P_gen_lost does not exist.")
		   end
		   ...
		   try
			   ...
			   CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_P_load_lost.csv"), output_CM_PtG_P_load_lost, append = true)
		   catch e
			   println("CM_PtG_P_load_lost does not exist.")
		   end
		   ...
		   output_CM_PtG_price = DataFrame(time=Int64[],node=Int64[],price=Float64[])
	   	    try
	   	    	...
	   	    	CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_price.csv"), output_CM_PtG_price, append = true)
	   	    catch e
	   	    	println("CM_PtG_price does not exist.")
	   	    end
			...
			try
				...
				CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_P_S_up.csv"), output_CM_PtG_P_S_up_opt, append = true)
			catch e
				println("CM_PtG_P_S_up_opt does not exist.")
			end
			...
			# Electricity generation from synthetic methane
		   	...
		   	try
		   		...
		   		CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_P_syn.csv"), output_CM_PtG_P_syn_opt, append = true)
		   	catch e
		   		println("CM_PtG_P_syn_opt does not exist.")
		   	end
			...
			# Electricity demand from PtG units
		   	...
		   	try
		   		...
		   		CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_D_PtG.csv"), output_CM_PtG_D_PtG_opt, append = true)
		   	catch e
		   		println("CM_PtG_D_PtG_opt does not exist.")
		   	end
			...
			# Synthetic methane storage level
		   	...
		   	try
		   		...
		   		CSV.write(joinpath("output", session*"__"*case*"__"*timeperiod_str, "CM_PtG_L_syn.csv"), output_CM_PtG_L_syn_opt, append = true)
		   	catch e
		   		println("CM_PtG_L_syn_opt does not exist.")
		   	end
