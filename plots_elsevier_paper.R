...
##############################
### BNetzA redispatch data ###
##############################

p_bnetza_redispatch_cost <- ggplot(data_bnetza_redipatch_cost) +
  plot_theme +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0), lim = c(0,730)) +
  geom_bar(stat = "identity", position = "dodge", aes(x = Year, y = Volume, fill = Measure)) +
  ylab("Cost (M. EUR)") +
  scale_fill_manual(values = c(fcolors["WindOnshore"][[1]], fcolors["Oil(heavy)"][[1]]), name = "") + 
  plot_legend

p_bnetza_redispatch_cost 


if (exists("p_bnetza_redispatch_cost")) {
  ggsave(filename = paste(session_dir[1], "plots_paper/", "plot_bnetza_redispatch_cost.pdf", sep = ""), 
         plot = p_bnetza_redispatch_cost, device = "pdf", width = 0.5*col_width, height = 5, units = "cm", dpi = 150)
  
  ggsave(filename = paste(session_dir[1], "plots_paper/png/", "plot_bnetza_redispatch_cost.png", sep = ""), 
         plot = p_bnetza_redispatch_cost, device = "png", width = col_width, height = 5, units = "cm", dpi = 150)
}

p_bnetza_redispatch_volume <- ggplot(data_bnetza_redipatch_volume) +
  plot_theme +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0), lim  = c(0, 19.2)) +
  geom_bar(stat = "identity", position = "dodge", aes(x = Year, y = Volume, fill = Measure)) +
  ylab("Volume (TWh)") +
  scale_fill_manual(values = c(fcolors["WindOnshore"][[1]], fcolors["Oil(heavy)"][[1]]), name = "") + 
  plot_legend

p_bnetza_redispatch_volume 


if (exists("p_bnetza_redispatch_volume")) {
  ggsave(filename = paste(session_dir[1], "plots_paper/", "plot_bnetza_redispatch_volume.pdf", sep = ""), 
         plot = p_bnetza_redispatch_volume, device = "pdf", width = 0.5*col_width, height = 5, units = "cm", dpi = 150)
  
  ggsave(filename = paste(session_dir[1], "plots_paper/png/", "plot_bnetza_redispatch_volume.png", sep = ""), 
         plot = p_bnetza_redispatch_volume, device = "png", width = col_width, height = 5, units = "cm", dpi = 150)
}
